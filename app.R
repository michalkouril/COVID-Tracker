        ...
        tabPanel("Plots",
               sidebarLayout(  
                sidebarPanel(
                  ...
                  #checkboxInput("relPop", "Adjust for population size"),
                  conditionalPanel(
...
server <- function(input, output, session) {
  ...
  output$plot1 <- renderPlot({
    ...
    if(input$yScale == 2){ #Only relevant when aligned by number of starting cases
      ...
      pop = mean(plot.data() %>% group_by(region) %>% summarise(p = max(population)) %>% pull(p))
      
      #Generate the doubline time using the doubleRate function (see at top)
      if(input$yScale == 2){
        plot = plot + 
          stat_function(fun = ~log10(doubleRate(.x, startCases, 2, 10000)),
                        linetype="dashed", colour = "#8D8B8B", size = 1.0, alpha = 0.3) +
          stat_function(fun = ~log10(doubleRate(.x, startCases, 3, 10000)),
                        linetype="dashed", colour = "#8D8B8B", size = 1.0, alpha = 0.3)
      } else {
        plot = plot + 
